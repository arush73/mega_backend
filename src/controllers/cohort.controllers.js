...
const addCohort = asyncHandler(async (req, res) => {
 ...
 if (!validate.success)
  ...
  return res.status(201).json(new ApiResponse(201,cohort,"Cohort added successfully"));
})
...
const updateCohort = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, cohort,"Cohort updated successfully"));
})
...
const deleteCohort = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, deleteCohort,"Cohort deleted successfully"));
})
...
const getCohort = asyncHandler(async (req, res) => {
  ...
  if (!cohort) throw new ApiError(404, "Cohort not found")
  return res.status(200).json(new ApiResponse(200, cohort,"Cohort retrieved successfully"));
})

const listCohorts = asyncHandler(async (req, res) => {
  const cohorts = await Cohort.find()
  if (!cohorts) throw new ApiError(404, "Cohorts not found")
  return res.status(200).json(new ApiResponse(200, cohorts,"Cohorts retrieved successfully"));
})
